...
@app.post("/feed/upload", summary="Upload a single video frame for face detection")
async def upload_frame(file: UploadFile = File(...)):
    """
    Accepts a PNG/JPEG image frame.
    Runs face detection, stores ROI in DB, returns frame_id + ROI data.
    """
    ...
    record = save_roi(frame_id=frame_id, timestamp=timestamp, roi=roi)
...
async def get_frame(frame_id: str):
    ...
    """
    record = get_roi_by_frame(frame_id)
    if not record:
